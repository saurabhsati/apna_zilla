    @extends('web_admin.template.admin')


    @section('main_content')
    <!-- BEGIN Content -->
            <div id="main-content">
                <!-- BEGIN Page Title -->
                <div class="page-title">
                    <div>
                        <h1><i class="fa fa-credit-card"></i> Bulk Request</h1>
                        <h4>Request Deatils</h4>
                    </div>
                </div>
                <!-- END Page Title -->

                <!-- BEGIN Breadcrumb -->
                <div id="breadcrumbs">
                    <ul class="breadcrumb">
                        <li>
                            <i class="fa fa-home"></i>
                            <a href="{{ url('web_admin/dashboard') }}">Dashboard</a>
                            <span class="divider"><i class="fa fa-angle-right"></i></span>
                            <i class="fa fa-credit-card"></i>
                            <a href="{{ url('web_admin/deals_bulk_request') }}"> Bulk Request</a>
                            <span class="divider"><i class="fa fa-angle-right"></i></span>
                        </li>
                    
                    </ul>
                </div>
                <!-- END Breadcrumb -->

                <!-- BEGIN Main Content -->
                <div class="row">
                    <div class="col-md-12">
                       @if(isset($arr_single_request) && sizeof($arr_single_request)>0)
                        <div class="box">
                            <div class="box-content">
                                <div class="invoice">
                                    <div class="row">
                                        <div class="col-md-6">
                                            <h2>
                                            {{isset($arr_single_request['name']) ? ucfirst($arr_single_request['name']):'' }}

                                          </h2>
                                        </div>
                                        <div class="col-md-6 invoice-info">
                                            <p class="font-size-17">
                                            <strong>Date</strong> -
                                            {{ date('d M Y',strtotime($arr_single_request['created_at'])) }}</p>
                                        </div>
                                    </div>

                                    <hr class="margin-0" />

                                    <div class="row">
                                        <div class="col-lg-12 company-info">
                                            

                                            <br/>
                                            <p><i class="fa fa-envelope"></i>
                                                {{ isset($arr_single_request['email']) ?
                                                $arr_single_request['email']:'' }}
                                            </p>
                                            <p><i class="fa fa-phone"></i>
                                              {{ isset($arr_single_request['phone_no']) ?
                                                $arr_single_request['phone_no']:'' }}
                                             </p>


                                        </div>

                                    </div>

                                    <br/><br/>

                                    <div class="table-responsive">
                                        <table class="table table-striped table-bordered">
                                            <thead>
                                                <tr>
                                                    <th class="center">#</th>
                                                   
                                                    <th>Deal Name</th>
                                                    <th>Name</th>
                                                    <th>Email</th>
                                                    <th>Phone Number</th>
                                                    <th>Quantity</th>
                                                   
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <td class="center">1</td>
                                                   
                                                   <td> {{ isset($arr_single_request['order_deal']) ? ucfirst($arr_single_request['order_deal']['title']):'' }}</td>
                                                     <td>{{ $arr_single_request['name'] or ''}} </td>
                                                     <td>{{ $arr_single_request['email'] or ''}} </td>
                                                     <td>{{ $arr_single_request['phone_no'] or ''}} </td>
                                                    <td>{{ $arr_single_request['quantity'] or ''}} </td>
                                                </tr>

                                            </tbody>
                                        </table>
                                    </div>
                                    <br/>
                                </div>
                            </div>
                        </div>
                    @else
                        <h4>No Record Found!</h4>
                    @endif
                 </div>
                 </div>
                <!-- END Main Content -->


@stop