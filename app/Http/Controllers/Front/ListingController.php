...
class ListingController extends Controller
{
    ...
    public function list_details($city, $slug_area, $enc_id)
    {
        ...
        $obj_business_details = BusinessListingModel::where(['id' => $id, 'is_active' => '1'])->with(['business_times', 'also_list_category', 'image_upload_details', 'payment_mode', 'category_details', 'service', 'reviews' => function ($query) {
        ...
        if ($arr_business_details) {
            ...
            if (Session::has('user_id')) {
                ...
                $str = '';
                $obj_favourite = FavouriteBusinessesModel::where(['user_id' => $user_id, 'is_favourite' => '1'])->get(['business_id']);
        ...
        $obj_business_rating_count = BusinessListingModel::with(['reviews' => function ($query) {
    ...
    public function store_reviews(Request $request)
    {
        ...
        if ($status) {
            ...
            $reviews = 0;
            if (isset($business_rating[0]['reviews']) && count($business_rating[0]['reviews']) > 0) {
                foreach ($business_rating[0]['reviews'] as $business_review) {
                    $reviews = $reviews + $business_review['ratings'];
                }

            }
            if (count($business_rating[0]['reviews'])) {
                $tot_review = count($business_rating[0]['reviews']);
...
*/
        ...
        $obj_fav = FavouriteBusinessesModel::where(['user_id' => $user_id, 'business_id' => $business_id])->get();
        if ($obj_fav) {
            $arr = $obj_fav->toArray();
            if (count($arr) > 0) {
                if ($arr[0]['is_favourite'] == '0') {
                    $result = FavouriteBusinessesModel::where(['user_id' => $user_id, 'business_id' => $business_id])->update(['is_favourite' => '1']);
                    $json['status'] = 'favorites';
                }

                if ($arr[0]['is_favourite'] == '1') {
                    $result = FavouriteBusinessesModel::where(['user_id' => $user_id, 'business_id' => $business_id])->update(['is_favourite' => '0']);
                    $json['status'] = 'un_favorites';
                }
            } else {
                $result = FavouriteBusinessesModel::create(['user_id' => $user_id, 'business_id' => $business_id, 'is_favourite' => '1']);
                $json['status'] = 'favorites';
            }
        }
    ...
    public function send_enquiry(Request $request)
    {
        ...
        if ($status) {
            ...
            if ($obj_email_template) {
                ...
                // echo $content;exit;
                $send_mail = Mail::send([], [], function ($message) use ($email, $arr_email_template, $content, $t_email) {
