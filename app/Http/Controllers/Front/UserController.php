...
class UserController extends Controller
{
    ...
    public function store_personal_details(Request $request)
    {
        ...
        */
        ...
        if ($request->hasFile('profile_pic')) {
            $profile_pic_valiator = Validator::make(['profile_pic' => $request->file('profile_pic')], ['profile_pic' => 'mimes:jpg,jpeg,png']);
    ...
    public function my_business(Request $request, $page = '1')
    {
        ...
        $obj_business_info = BusinessListingModel::orderBy('created_at', 'DESC')->with(['category', 'membership_plan_details.membership'])->where('user_id', '=', $user_id)->with('reviews');
    ...
    public function get_zip(Request $request)
    {
        ...
        $json = [];
        $obj_result = PlaceModel::where(['city_id' => $city_id])->distinct('postal_code')->get(['id as zip_id', 'postal_code']);
        if ($obj_result) {
            $json = $obj_result->toArray();
        }
    ...
    public function add_business_details(Request $request)
    {
        ...
        if ($request->hasFile('business_image')) {
            $profile_pic_valiator = Validator::make(['business_image' => $request->file('business_image')], ['business_image' => 'mimes:jpg,jpeg,png']);
    ...
    public function add_other_details(Request $request)
    {
        ...
        if (count($arr_payment) > 0) {
            foreach ($arr_payment as $key => $value) {
                BusinessPaymentModeModel::create(['business_id' => $business_id, 'title' => $value]);
            }
        }
    ...
    public function add_services_details(Request $request)
    {
        ...
        if (count($business_services) > 0) {
            foreach ($business_services as $key => $value) {
                BusinessServiceModel::create(['name' => $value, 'business_id' => $business_id]);
            }
        }
...
*/
    ...
    public function my_order(Request $request, $page = '1')
    {
        ...
        if (Session::has('user_id')) {
            $id = session('user_id');
            $user_id = base64_decode(Session::get('user_id'));
            $arr_order = $arr_paginate_my_order = [];
            $per_page = 2;
