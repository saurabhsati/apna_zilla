...
class VenderController extends Controller
{
        ...
        if (is_numeric($mobile_no) && strlen($mobile_no) == 10) {
            if ($email != '') {
                ...
                if ($obj_email_arr_count) {
                    $email_exist = count($obj_email_arr_count->toArray());
                }
                if ($email_exist > 0) {
                    $json['status'] = 'ERROR';
            ...
            if ($mobile_no != '') {
                ...
                if ($obj_mobile_arr_count) {
                    $mobile_exist = count($obj_mobile_arr_count->toArray());
                }
    ...
    public function update(Request $request)
    {
        ...
        $profile_pic = false;
        if ($request->hasFile('profile_pic')) {
            $cv_valiator = Validator::make(['profile_pic' => $request->file('profile_pic')], [
                'profile_pic' => 'mimes:jpg,jpeg,png',
            ]);
    ...
    public function multi_action(Request $request)
    {
        ...
        if (isset($checked_record_arr) && count($checked_record_arr) > 0) {
            //dd($multi_action);
