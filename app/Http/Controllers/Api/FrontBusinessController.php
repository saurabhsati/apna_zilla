...
class FrontBusinessController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //dd($business_listing);
        $arr_data = [];
        if (isset($business_listing) && count($business_listing) > 0) {
            foreach ($business_listing as $key => $business) {
                ...
                if (isset($business['category']) && count($business['category']) > 0) {
                ...
                $arr_data[$key]['review_star_count'] = count($business['reviews']);
                ...
                $arr_data[$key]['membership_plan'] = count($business['membership_plan_details']);
    ...
    public function store_business_step1(Request $request)
    {
        ...
        BusinessListingModel::where('id', '=', $business_id)->update(['busiess_ref_public_id' => $public_id]);

        $insert_data = 0;

        if (isset($business_cat) && count($business_cat) > 0) {
    ...
    public function store_business_step2(Request $request)
    {
        ...
        $location_add = BusinessListingModel::where(['user_id' => $user_id, 'id' => $business_id])->update($arr_data);
    ...
    public function store_business_step4(Request $request)
    {
        ...
        $insert_data = BusinessListingModel::where(['user_id' => $user_id, 'id' => $business_id])->update($arr_data);

        if (isset($payment_mode) && count($payment_mode) > 0) {
    ...
    public function store_business_step5(Request $request)
    {
        ...
        if (isset($business_service) && count($business_service) > 0) {
    ...
    public function edit_business_step(Request $request)
    {
        ...
        if (isset($business_data) && count($business_data) > 0) {
            /* Step 1 Data */
            ...
            if (isset($business_data['category']) && count($business_data['category']) > 0) {
            ...
            if (isset($business_data['business_times']) && count($business_data['business_times']) > 0) {
            ...
            if (isset($business_data['payment_mode']) && count($business_data['payment_mode']) > 0) {
            ...
            if (isset($business_data['image_upload_details']) && count($business_data['image_upload_details']) > 0) {
            ...
            $services_array = [];
            if (isset($business_data['service']) && count($business_data['service']) > 0) {
    ...
    public function update_business_step1(Request $request)
    {
        ...
        if ($request->hasFile('main_image')) {
            $profile_pic_valiator = Validator::make(['main_image' => $request->file('main_image')], ['main_image' => 'mimes:jpg,jpeg,png']);
    ...
    public function favorite(Request $request)
    {
        ...
        $arr_data = [];
        if (isset($fav_business_listing) && count($fav_business_listing) > 0) {
            foreach ($fav_business_listing as $key => $fav) {
                ...
                $arr_data[$key]['review_star_count'] = count($fav['business'][0]['reviews']);
    ...
    public function toggle_favourite(Request $request)
    {
        ...
        $obj_fav = FavouriteBusinessesModel::where(['user_id' => $user_id, 'business_id' => $business_id])->first();
        if ($obj_fav) {
            $arr_fav = $obj_fav->toArray();
        }

        if (isset($arr_fav) && count($arr_fav) > 0) {
            /*foreach ($arr_fav as $key => $fav)
            {*/

            if ($arr_fav['is_favourite'] == '0') {
                $result = FavouriteBusinessesModel::where(['user_id' => $user_id, 'business_id' => $business_id])
                    ->update(['is_favourite' => '1']);
                $json['status'] = 'SUCCESS';
                $json['message'] = 'Business Favorite Successfully  !';
            } elseif ($arr_fav['is_favourite'] == '1') {
                $result = FavouriteBusinessesModel::where(['user_id' => $user_id, 'business_id' => $business_id])
                    ->update(['is_favourite' => '0']);
            ...
            $result = FavouriteBusinessesModel::where(['user_id' => $user_id, 'business_id' => $business_id])
                ->insert(['user_id' => $user_id, 'business_id' => $business_id, 'is_favourite' => '1']);
            $json['status'] = 'SUCCESS';
