...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($obj_user != false) {
            $vender_count = count($obj_user->toArray());
        }
        ...
        if ($obj_sales_executive != false) {
            ...
            $sales_executive_count = count($obj_sales_executive->toArray());
        }

        $obj_business_listing = BusinessListingModel::get();
        if ($obj_business_listing) {
            $business_listing_count = count($obj_business_listing->toArray());
        }

        $obj_deal = DealsOffersModel::get();
        if ($obj_deal) {
            $deals_count = count($obj_deal->toArray());
        }
    ...
    public function view_sales_activity(Request $request)
    {
        ...
        [$month, $day, $year] = explode('/', $from_date);
        $from = $year.'-'.$month.'-'.$day;
        [$month, $day, $year] = explode('/', $to_date);
        $to = $year.'-'.$month.'-'.$day;
