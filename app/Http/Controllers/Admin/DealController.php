...
class DealController extends Controller
{
        ...
        if (count($transaction_details) > 0) {
            ...
            if (strpos($difference, 'after') !== false || strpos($difference, 'today') !== false) {
                ...
                if ($no_of_deals == 'unlimited') {
                    $add_deal = 1;
                } elseif ($no_of_deals > $total_deal_count) {
                    $add_deal = 1;
                } else {
                    $add_deal = 0;
                }
                ...
                $deal_ids = [];
                if (count($arr_deals) > 0 && isset($arr_deals)) {
    ...
    public function create($enc_id)
    {
        ...
        if (count($obj_business_category) > 0) {
        ...
        return view('web_admin.deal.create', compact('page_title', 'arr_business', 'parent_category_id', 'expired_date'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('deal_image')) {///image loaded/Not
            ...
            $image_validate = Validator::make(['deal_image' => $request->file('deal_image')],
                ['deal_image' => 'mimes:jpg,jpeg,png']);
    ...
    protected function _activate($enc_id)////$enc_id = Deal id
    {
        ...
        return DealModel::where('id', $id)->update(['is_active' => 1]);
    }
    ...
    protected function _block($enc_id)////$enc_id = Deal id
    {
        ...
        return DealModel::where('id', $id)->update(['is_active' => 0]);
    }
