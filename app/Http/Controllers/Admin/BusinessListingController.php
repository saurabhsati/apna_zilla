...
class BusinessListingController extends Controller
{
    ...
    public function update(Request $request, $enc_id)
    {
        ...
        if (count($business_cat) > 0 && $business_cat != null) {
    ...
    public function show($enc_id)
    {
        ...
        $business_data = [];
        $business_data = BusinessListingModel::with(['category', 'user_details', 'reviews', 'membership_plan_details', 'business_times', 'service', 'image_upload_details', 'payment_mode'])->where('id', $id)->get()->toArray();
    ...
    public function get_plan_cost(Request $request)
    {
        ...
        if (count($arr_membership_plan) > 0) {
            $arr_cost_data = [];
            $obj_cost_data = MemberCostModel::where('category_id', $category_id)->first();
            if ($obj_cost_data) {
                $arr_cost_data = $obj_cost_data->toArray();
