 ...
 */
function parseExportParams($strParams, $exportRequestStream = [])
{
...
function getExporter($strFormat, $streamtype = 'RLE')
{
    ...
    $associationCluster = bang(HANDLER_ASSOCIATIONS, ['|', ':'], true);
    $associations = bang(@$associationCluster[$streamtype], [';', '='], true);
...
{
    exit(buildResponse(parseExportedStatus($status, $meta, $msg)));
}
...
function setupDownload($exportFile, $exportType, $target = '_self')
{
    ...
    return ['ready' => 'download', 'type' => $exportType];
}
 ...
 */
function bang($str, $delimiterList = [';', '='], $retainPropertyCase = false)
{
